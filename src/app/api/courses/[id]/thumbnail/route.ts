...
async function resolveCourse(courseId: string) {
  ...
  const { data: course } = await adminDb
    .from("courses")
    .select("id, college_id, thumbnail_url, thumbnail_storage_path")
    .eq("id", courseId)
    .single();
...
export async function POST(
  ...
  const rl = checkRateLimit(`thumbnail-upload:${ip}`, LIMITS.mediaUpload.limit, LIMITS.mediaUpload.windowMs);
  ...
  if (course.thumbnail_storage_path) {
    await adminDb.storage.from("course-media").remove([course.thumbnail_storage_path]);
  }
  ...
  if (storageErr) {
    console.error("[thumbnail upload] storage:", storageErr.message);
  ...
  const { error: updateErr } = await adminDb
    .from("courses")
    .update({ thumbnail_url: publicUrl, thumbnail_storage_path: storagePath })
    .eq("id", courseId);

  if (updateErr) {
    await adminDb.storage.from("course-media").remove([storagePath]);
    console.error("[thumbnail upload] db:", updateErr.message);
    return NextResponse.json({ error: updateErr.message }, { status: 500 });
  }

  await writeAudit(adminDb, "upload_thumbnail", courseId, course.college_id, user.id, storagePath, file.size);

  return NextResponse.json({ thumbnail_url: publicUrl }, { status: 200 });
}
...
export async function DELETE(
  ...
  const { error } = await adminDb
    .from("courses")
    .update({ thumbnail_url: null, thumbnail_storage_path: null })
    .eq("id", courseId);

  if (error) {
    console.error("[thumbnail delete] db:", error.message);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }

  await writeAudit(adminDb, "delete_thumbnail", courseId, course.college_id, user.id, null, null);
