...
export async function POST(
  ...
  const rl = checkRateLimit(`cover-upload:${ip}`, LIMITS.mediaUpload.limit, LIMITS.mediaUpload.windowMs);
  ...
  const storagePath = `${collegeId}/cover/${Date.now()}.${ext}`;
  ...
  if (storageErr) {
    console.error("[cover upload] storage:", storageErr.message);
  ...
  if (updateErr) {
    ...
    console.error("[cover upload] db:", updateErr.message);
    return NextResponse.json({ error: updateErr.message }, { status: 500 });
  }

  return NextResponse.json({ cover_image_url: publicUrl }, { status: 200 });
}
