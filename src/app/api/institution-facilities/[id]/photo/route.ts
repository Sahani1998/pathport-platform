...
export async function POST(
  ...
  const storagePath = `${row.college_id}/facilities/${id}/${Date.now()}.${ext}`;
  ...
  if (row.cover_storage_path && row.cover_storage_path !== storagePath) {
    await adminDb.storage.from("institution-media").remove([row.cover_storage_path]);
  }

  const { error: updateErr } = await adminDb
    .from("institution_facilities")
    .update({ cover_storage_path: storagePath, cover_image_url: publicUrl })
    .eq("id", id);
  if (updateErr) {
