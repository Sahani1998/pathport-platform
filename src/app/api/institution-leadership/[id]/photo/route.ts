...
export async function POST(
  ...
  const rl = checkRateLimit(`photo-upload:${ip}`, LIMITS.photoUpload.limit, LIMITS.photoUpload.windowMs);
  ...
  const storagePath = `${row.college_id}/leadership/${id}/${Date.now()}.${ext}`;
  ...
  // Remove old photo file if it differs
  if (row.photo_storage_path && row.photo_storage_path !== storagePath) {
    await adminDb.storage.from("institution-media").remove([row.photo_storage_path]);
  }

  const { error: updateErr } = await adminDb
    .from("institution_leadership")
    .update({ photo_storage_path: storagePath, photo_url: publicUrl })
    .eq("id", id);
  if (updateErr) {
