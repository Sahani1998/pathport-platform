...
import type { LeadershipMember, PersonStatus } from "@/types/institution-people";
...
export async function PATCH(
  ...
  const { data: row, error: rowErr } = await adminDb
    .from("institution_leadership").select("id, college_id, status").eq("id", id).maybeSingle();
  ...
  if ("role"       in body) patch.role       = (body.role as string)?.trim()       || null;
  ...
  const { data: updated, error: updateErr } = await adminDb
    .from("institution_leadership").update(patch).eq("id", id).select().single();
  if (updateErr) return NextResponse.json({ error: updateErr.message }, { status: 500 });

  return NextResponse.json({ member: updated as LeadershipMember });
}
...
export async function DELETE(
  ...
  const adminDb = createAdminClient();
  const { data: row, error: rowErr } = await adminDb
    .from("institution_leadership").select("id, college_id, photo_storage_path").eq("id", id).maybeSingle();
  if (rowErr || !row) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (profile.role === "institution" && row.college_id !== profile.college_id) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  const { error: deleteErr } = await adminDb.from("institution_leadership").delete().eq("id", id);
