...
export const metadata = { title: "Campus Gallery — Institution" };
...
export default async function GallerySettingsPage() {
  ...
  const { data: mediaRows } = await adminDb
    .from("institution_media")
    .select("*")
    .eq("college_id", profile.college_id)
    .eq("media_type",  "gallery_image")
    .order("status")          // published first (alphabetically before draft)
    .order("sort_order")
    .order("created_at", { ascending: false });

  const items = (mediaRows ?? []) as InstitutionMedia[];
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <div>
        ...
        <h2 className="font-display text-3xl text-white mb-1">Campus Gallery</h2>
        <p className="text-white/40 font-body text-sm">
          Upload photos of your campus, facilities, events, and student life. Published images appear on your public college page.
        </p>
      </div>

      <GalleryManager initialItems={items} collegeId={profile.college_id} />
    </div>
  );
}
