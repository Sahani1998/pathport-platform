...
export type DestinationStatus = "live" | "coming_soon" | "hidden";
...
export default function DestinationsClient({ initialRows }: { initialRows: Destination[] }) {
  const [rows,         setRows]         = useState<Destination[]>(initialRows);
      ...
      <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-20 gap-3 text-white/35">
            ...
            <span className="font-body text-sm">Refreshing destinations…</span>
          </div>
        ) : rows.length === 0 && !error ? (
