-- Sprint 20A — Analytics correctness fix
--
-- Adds `tuition_fee_payment_pending` (Sprint 19 stage) to the "approved"
-- bucket inside three analytics RPCs. Without this fix, an applicant who
-- has been IPA-approved but is awaiting tuition payment shows as NOT
-- approved in institution dashboards and conversion rates.
--
-- Mirrors APPROVED_STAGES in src/lib/application-workflow.ts.
--
-- All three functions are CREATE OR REPLACE — safe to re-run.
-- No new tables, no new columns, no new permissions.
-- Re-grants are intentional (CREATE OR REPLACE preserves grants, but we
-- restate them for clarity).

-- ═════════════════════════════════════════════════════════════════════════════
-- A. get_admin_analytics_summary()
--    Stage breakdown is by exact stage — no aggregation change needed here,
--    but we keep the signature identical and refresh the function body so
--    a single SQL run is enough to deploy Sprint 20A.
-- ═════════════════════════════════════════════════════════════════════════════
...
--    APPROVED_STAGES now includes 'tuition_fee_payment_pending'.
--    Affects `approved_applications` count + `conversion_rate`.
-- ═════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'approved_applications',  (
      -- Mirrors APPROVED_STAGES in src/lib/application-workflow.ts (Sprint 20A: + tuition_fee_payment_pending)
      SELECT COUNT(*)::int FROM apps
      WHERE stage IN ('approved','tuition_fee_payment_pending','arrival_preparation',
...
--    APPROVED_STAGES in approvals_per_month CTE — same fix.
-- ═════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  approvals_per_month AS (
    -- Mirrors APPROVED_STAGES in src/lib/application-workflow.ts (Sprint 20A: + tuition_fee_payment_pending)
    SELECT month,
           COUNT(*) FILTER (WHERE stage IN ('approved','tuition_fee_payment_pending','arrival_preparation','arrived_singapore',
