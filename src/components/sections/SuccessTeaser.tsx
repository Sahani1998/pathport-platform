...
export default async function SuccessTeaser() {
  ...
  return (
    <section id="success-stories" className="relative py-20 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-10">
        ...
        {stories.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
            {stories.map((s, i) => (
              <Reveal key={s.id} delay={i * 80} className="h-full">
                <div className="flex flex-col h-full p-6 rounded-2.5xl bg-white/[0.04] border border-white/[0.08]">
                  <div className="flex items-center gap-3 mb-4">
                    {s.photo_url ? (
                      <Image
                        ...
                        alt={s.person_name}
                        width={44} height={44}
                        className="w-11 h-11 rounded-full object-cover border border-white/[0.10] flex-shrink-0"
                        unoptimized
                      />
                    ) : (
                      <div className="w-11 h-11 rounded-full bg-gold-400/10 border border-gold-400/20 flex items-center justify-center flex-shrink-0">
                  ...
                  {s.story_text && (
                    <p className="text-white/55 font-body text-sm leading-relaxed mb-4 flex-1 line-clamp-4">
          ...
          <Reveal className="max-w-2xl mx-auto mb-10 p-7 rounded-2.5xl bg-white/[0.04] border border-white/[0.08] text-center">
            <div className="w-11 h-11 rounded-2xl bg-gold-400/10 border border-gold-400/25 text-gold-400 flex items-center justify-center mx-auto mb-4">
              <Quote className="w-5 h-5" />
            </div>
            <p className="font-display text-xl text-white mb-2 leading-snug">
